...
async fn custom_fractal_test() {
    ...
    //Mandelbrot
    let req = actix_web::test::TestRequest::with_uri("/fractals/Custom?colors=ffffff,11ffff,1100ff&position_x=-.1&position_y=1&zoom=10&debug=true&width=1024&height=1024&max_iterations=2000&num_colors=2000")
        .to_request();
    let resp = actix_web::test::call_service(&mut app, req).await;
    let status = resp.status();
    assert_eq!(status, actix_web::http::StatusCode::OK);
